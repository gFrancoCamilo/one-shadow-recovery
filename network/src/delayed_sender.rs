...
pub struct DelayedSender {
    ...
    rng: SmallRng,
    /// Firewall indicates which nodes the current node is allowed to talk (helps us implement
    /// cliques and twins)
    pub firewall: HashMap<u64, Vec<SocketAddr>>,
    /// This indicates when nodes in different cliques are allowed to communicate
    pub allow_communications_at_round: u64,
    pub network_delay: u64,
    /// DNS is only used for AWS experiments. It translates the AWS address to adequate to local
    /// logic
...
impl DelayedSender {
    ...
    pub async fn send(&mut self, address: SocketAddr, data: Bytes, current_round: u64) {
        ...
        // We only send the mesages to nodes in our clique
