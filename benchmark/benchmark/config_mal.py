...
class Committee:
    def __init__(self, names, consensus_addr, transactions_addr, mempool_addr, twins):
        ...
        # Setting up ports for nodes to communicate with node
        #self.clients_listen = client_listen
        self.twins = twins
    ...
    def _build_consensus(self):
        ...
        faults = int((self.twins/2)**0.5)
        try:
            with open('./benchmark/.env-info.json', 'r') as f:
                data = load(f)
                faults = data['num_honest'] - 1
                total_nodes = data['nodes']
        except FileNotFoundError:
            print('\033[1;33;48m WARNING\033[1;37;0m: File .env-info.json does not exist. Continuing with standard calculation.\
                    This may result in wrong results. Run the setup-env.py script beforehad to remove this warning.')
        return {'authorities': node, 'epoch': 1, 'num_of_twins': self.twins, 'faults': faults, 'nodes': total_nodes}
