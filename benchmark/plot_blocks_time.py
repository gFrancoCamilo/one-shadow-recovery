    ...
    for node_id in range(num_nodes):
        ...
        if os.path.exists(log_filename):
            with open(log_filename, 'r') as log_file:
                ...
                end_sync_match = search(r'\[(.*Z) .* Finished retrieva.', log_content)
                ...
                if start_sync_match and end_sync_match:
                    if alt_chain_match:
                        if int(alt_chain_match.group(1)) > 10:
                ...
                if alt_chain_match:
                        if int(alt_chain_match.group(1)) > 10:
                            alt_chain_lengths.append(math.log(int(alt_chain_match.group(1)), 2))  # Store the number after the phrase
                else:
...
def plot_avg_sync_time_vs_blocks(experiment_dir='experiment_logs', output_file='blocks_vs_time.pdf'):
    ...
    for a_dir in os.listdir(experiment_dir):
        ...
        if os.path.isdir(a_dir_path):
            ...
            avg_block_times = group_by_alt_chain_length(alt_chain_lengths, start_sync, end_sync)
    ...
    plt.xlabel('Number of Blocks (log scale)', fontsize=14)
