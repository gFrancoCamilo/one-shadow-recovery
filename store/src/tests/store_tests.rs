...
#[tokio::test]
async fn read_write_delete_value() {
    // Create new store.
    let path = ".db_test_read_write_delete_value";
    let _ = fs::remove_dir_all(path);
    let mut store = Store::new(path).unwrap();
    ...
    // Read value.
    let result = store.read(key.clone()).await;
    assert!(result.is_ok());
    let read_value = result.unwrap();
    assert!(read_value.is_some());
    assert_eq!(read_value.unwrap(), value);
    ...
    let result_delete = store.read(key).await;
    assert!(result_delete.is_ok());
    let deleted_read = result_delete.unwrap();
